...
def foodHeuristic(state, problem):
    ...
    if problem.isGoalState(state):
        return 0
    dist_set = set()  # ena set apo tis apostaseis gia kathe food syntetagmeni

    for i in foodGrid.asList():  # prospelazw th lista tou foodgrid
        prob = PositionSearchProblem(  # ftiaxnw to problem pou tha xreiastw gia to A* gia ton ypologismo ths apostasis opws ginetai kai stis alles
            problem.startingGameState, start=position, goal=i, warn=False, visualize=False)
        # ypologizw to length apo ton A* algorithmo anazitisis tou provlimatos parapanw
        dist = len(search.aStarSearch(prob, manhattanHeuristic))
        dist_set.add(dist)  # to vazw sto set
    return max(dist_set)  # kai epistrefw to megisto apo to set
